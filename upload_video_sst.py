...
CLIENT_SECRETS_FILE = "client_secrets_sst.json"
    ...
    try:
      ...
      if response is not None:
        if 'id' in response:
          ...
          filepath = str(file)
          filepath = filepath.replace('video_layer', 'thumb')
          filepath = filepath.replace('mp4', 'png')

          url = "http://heemkim.ddns.net:8000/pybo/automake/youtube_upload_complete"
          request_api = urllib.request.Request(url)
          data = "video_id=" + response['id'] + "&filepath=" + filepath
...
if __name__ == '__main__':
  ...
  argparser.add_argument("--thumbnail", help="Video thumbnail", default="thmb")
